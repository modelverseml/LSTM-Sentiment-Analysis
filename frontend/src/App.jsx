...
export default function App() {
  ...
  const subtitle = info && info.encoders && info.frameworks
    ? `One review, ${plural(info.encoders.length, "embedding")} × ${plural(info.frameworks.length, "implementation")} of the same LSTM (${info.frameworks.map((f) => FRAMEWORK_NAME[f] || f).join(", ")}). Watch them compare.`
    : "One review, the same LSTM built several ways. Watch them compare.";

  return (
    <div className="page">
      <header className="hero">
        <h1>
          <span className="accent">LSTM</span> Sentiment
        </h1>
        <p className="sub">
          {subtitle}
        </p>
